...
def draw():    
    global recursions,recursion_depth, branch_angle, length_initial_trunk
    """--------------------------------------"""
    if (keyPressed):
        if (key == 'D'):
    ...
    recursive_tree(recursion_depth, recursions, length_initial_trunk, branch_angle)
...
def mousePressed():
    ...
    """ Draw the Tree """
def recursive_tree(recursion_depth, recursions, length_initial_trunk, branch_angle):
    hold_x = 400
    hold_y = 800

    line(400, 800, 400, 700-length_initial_trunk)
